      ...
      {cards.map((card) => {
        ...
        return (
          <Card key={card.label} className="border-white/12 bg-white/[0.02] p-5">
            <p className="text-xs uppercase tracking-[0.16em] text-accent-gold"><T text={card.label} /></p>
            <h3 className="mt-2 font-heading text-2xl leading-tight text-text-primary"><T text={product.name} /></h3>
            <p className="mt-3 text-sm leading-relaxed text-text-secondary"><T text={card.description} /></p>
            <div className="mt-4">
            ...
            <p className="mt-3 inline-flex items-center gap-2 text-xs uppercase tracking-[0.14em] text-text-secondary">
              <T text="Matches a specific buyer type" />
