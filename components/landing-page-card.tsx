...
export function LandingPageCard({ page }: LandingPageCardProps) {
  ...
  return (
    <Card className="group overflow-hidden border-white/12 bg-white/[0.02] transition-all duration-300 hover:-translate-y-1 hover:border-accent-gold/60">
      <LocalizedLink href={`/landing/${page.slug}`} className="block p-6">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <span className="inline-flex items-center gap-1 text-xs uppercase tracking-[0.14em] text-text-secondary">
            ...
            <T text={category?.name ?? page.categoryId} />
          </span>
        </div>

        <h3 className="mt-4 font-heading text-2xl leading-tight text-text-primary">
          <T text={page.title} />
        </h3>
        <p className="mt-3 text-sm leading-relaxed text-text-secondary">
          <T text={page.excerpt ?? page.description} />
        </p>

        <div className="mt-5 space-y-2">
          <p className="text-xs uppercase tracking-[0.16em] text-text-secondary">
            <T text="Priority order" />
          </p>
          {(page.priorityOrder?.slice(0, 3) ?? page.bestFor.slice(0, 3)).map((item) => (
